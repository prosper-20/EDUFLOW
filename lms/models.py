...
class Task(models.Model):
    ...
    allowed_file_types = models.ManyToManyField(
        FileType,
        blank=True,
    ...
    late_submission_penalty = models.FloatField(
        default=0.0, help_text="Percentage penalty per day (e.g., 5.0 for 5% per day)"
    )
    require_passing_score = models.BooleanField(default=False)
    passing_score = models.PositiveIntegerField(
        default=70, help_text="Minimum score needed to pass (if required)"
    )
...
class TaskSubmission(models.Model):
    ...
    file_upload = models.FileField(
        upload_to=task_submission_upload_path, blank=True, null=True
    )
...
class StudentTaskProgress(models.Model):
    ...
    task = models.ForeignKey(
        Task, on_delete=models.CASCADE, related_name="student_progress"
    )
...
class Classroom(models.Model):
    ...
    students = models.ManyToManyField(
        settings.AUTH_USER_MODEL, blank=True, limit_choices_to={"role": "Student"}
    )
